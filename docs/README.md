...
| [`aws.md`](aws.md) | **Deploying (ECS).** The AWS-native runbook: ECR → Fargate → ALB, Aurora, Secrets Manager, and the preflight gate the container runs before it serves. |
| [`k8s.md`](k8s.md) | **Deploying (Kubernetes).** The `landvex-prod` EKS runbook — grounded in a real infrastructure inventory, every gap and decision named. |
