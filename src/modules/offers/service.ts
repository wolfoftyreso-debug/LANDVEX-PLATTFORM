...
export async function submitOffer(
  ...
  // Freeze verification state at submission time
  const corridor = await getCorridorBySlug("lt-se");
  if (!corridor) throw new Error("Corridor not seeded");
  const facts = await getVerifiedFacts(companyId, corridor.id);
  const workerCerts = await getApprovedWorkerCerts(companyId, corridor.id);
...
export async function acceptOffer(actor: Actor, offerId: string): Promise<Offer> {
