...
async function main() {
  ...
  for (const [index, spec] of SUPPLIERS.entries()) {
    ...
    try {
      const owner = await registerUser({
        email: `supplier${index + 1}@demo.balticbridge.example`,
      ...
      const owner = await db.query.users.findFirst({
        where: eq(users.email, `supplier${index + 1}@demo.balticbridge.example`),
      });
      ownerId = owner?.id;
    }
    ...
    for (const item of items) {
      ...
      await transitionItem(ops, item.id, "approved");
    }
  ...
  const owner1 = await db.query.users.findFirst({
    where: eq(users.email, "supplier1@demo.balticbridge.example"),
  });
  const owner2 = await db.query.users.findFirst({
    where: eq(users.email, "supplier2@demo.balticbridge.example"),
  });
  if (owner1 && owner2) {
